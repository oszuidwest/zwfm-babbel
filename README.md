...
Babbel normalizes:
- The story audio, during the upload or the TTS process. Babbel runs `loudnorm` in two passes: the first measures the mono downmix, the second applies a linear gain from those measurements. The voice keeps its dynamics and lands on -16 LUFS with -1 dBTP as the ceiling. When a linear gain would exceed that ceiling or the loudness range is out of bounds, `loudnorm` falls back to dynamic mode.
