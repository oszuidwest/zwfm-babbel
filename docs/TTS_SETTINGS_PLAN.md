...
| 32 | Missing settings storage behavior | **New `repository.ErrSchemaUnavailable` sentinel** triggered on MySQL error 1146 (ER_NO_SUCH_TABLE). `gorm.ErrRecordNotFound` on the singleton row maps separately to `repository.ErrNotFound`. Service layer wraps both into `apperrors.NotInitializedError`, but with different problem codes and hints: missing table says to apply migration `005_tts_settings.sql`; missing row says to restore the seeded `id=1` row. Applies to **GET `/settings/tts`, PATCH `/settings/tts`, AND POST `/stories/{id}/tts`** | Loud, actionable failure mode without telling admins to re-run an already-applied migration when only the seed row was deleted |
...
Two error paths produce 503 responses with different problem codes and remediation hints. **Both `Get(ctx)` and `Update(ctx, ...)` go through this path** — meaning GET `/settings/tts`, PATCH `/settings/tts`, and POST `/stories/{id}/tts` all return 503 when the table or row is missing. `Update` calls `Get` internally first to detect the missing-row case before attempting the UPDATE.
...
- `internal/apperrors/errors.go` gets a new typed error **with an explicit `Error()` method and `Unwrap()`** so it satisfies the `error` interface (required for `errors.AsType[*apperrors.NotInitializedError]` to work):
  ...
  ```
- `TTSSettingsService.Get(ctx)` translates `repository.ErrSchemaUnavailable` into `apperrors.NotInitialized("tts_settings", "apply migration 005_tts_settings.sql", err)` and translates `repository.ErrNotFound` into `tts_settings.row_missing` with a re-seed hint.
...
503 missing-schema — on GET `/settings/tts`, PATCH `/settings/tts`, AND POST `/stories/{id}/tts`. The body is what `utils.ProblemExtended(c, 503, "tts_settings not initialized", "tts_settings.not_initialized", "apply migration 005_tts_settings.sql")` produces verbatim:
...
503 missing-row — same endpoints, but for a deleted singleton row after migration `005` has already run:

```json
{
  "type":      "https://babbel.api/problems/tts_settings.row_missing",
  "title":     "Service Unavailable",
  "status":    503,
  "detail":    "tts_settings singleton row missing",
  "instance":  "/api/v1/settings/tts",
  "timestamp": "2026-06-06T12:34:56Z",
  "code":      "tts_settings.row_missing",
  "hint":      "restore the id=1 row from migrations/005_tts_settings.sql seed data"
}
```
...
| `internal/apperrors/errors.go` | Add **two** new typed errors: `NotInitializedError` (with `Error()`/`Unwrap()`) and `ValidationProblemError` + `FieldValidationError` (with `Error()`). `ValidationProblemError` has a `Resource` field so the handler logs the correct resource regardless of caller (settings service OR story service). Factory functions `NotInitialized(...)` and `NewValidationProblemError(resource, detail, errs)` |
| `internal/services/tts_settings_service.go` | New — defines its own `UpdateTTSSettingsRequest` struct (with `Clear*` flags) at the service layer. `Get(ctx)` and `Update(ctx, *UpdateTTSSettingsRequest)`. **`Update` calls `Get` first** to detect missing-table/row, then translates the service-DTO → `repository.TTSSettingsUpdate` (mirrors `voice_service.go:64-68`), then calls the custom repo `Update` (whose 0-rows-affected outcome is treated as success, since same-value PATCH is valid). Both `Get` and the preliminary `Get` inside `Update` map `repository.ErrSchemaUnavailable` and `repository.ErrNotFound` to `apperrors.NotInitializedError`, but with distinct problem codes and hints. Range/enum/length validation in `Update` aggregates into `apperrors.NewValidationProblemError("tts_settings", ...)`. `pkg/logger.Info` audit. |
