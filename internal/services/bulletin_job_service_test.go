...
func TestResolveJobOutcome(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "state conflict through the transaction wrapper",
			ctx:  context.Background(),
			err:  apperrors.Database("Bulletin", "create", repository.ErrBulletinJobStateConflict),
			want: jobConflict,
		},
		{
...
func TestRunAttemptStopsWaitingForStationLockOnCancel(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Virtual time only advances once runAttempt is durably blocked on the
		// lock, so the cancel provably interrupts an in-progress wait.
...
func TestRunAttemptGeneratesAfterLockIsFreed(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		svc := newTestJobService(t)
		var generations int
		svc.generateBulletin = func(context.Context, int64, time.Time, func(context.Context, int64) error) (int64, error) {
		...
		bulletinID, err := svc.runAttempt(context.Background(), &models.BulletinJob{ID: 1, StationID: 7})
